<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        ...
        <div class="mb-8">
            ...
            @php
                $ttaxIncomes = $workingPaper->incomeItems->where('section_type', 'ttax');
            @endphp

            @if($ttaxIncomes->count() > 0)
            ...
            <div x-data="{ showIncomeForm: false }">
                ...
                <div x-show="showIncomeForm" x-cloak class="border rounded-lg p-4 bg-gray-50">
                    <form method="POST" action="{{ route('client.income.store', $workingPaper) }}" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="section_type" value="ttax">

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-500">*</span></label>
                                <input type="text" name="description" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g., Trust income">
                            </div>
        ...
        <div>
            ...
            @php
                $ttaxExpenses = $workingPaper->expenseItems->where('section_type', 'ttax');
            @endphp

            @if($ttaxExpenses->count() > 0)
                <div class="overflow-x-auto mb-4">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($ttaxExpenses as $expense)
                                <tr>
                ...
                <div x-show="showExpenseForm" x-cloak class="border rounded-lg p-4 bg-gray-50">
                    <form method="POST" action="{{ route('client.expense.store', $workingPaper) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-500">*</span></label>
                                <input type="text" name="description" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g., Trust expenses">
                            </div>
        ...
        <!-- Totals Summary -->
        @if($ttaxIncomes->count() > 0 || $ttaxExpenses->count() > 0)
            <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                <h5 class="font-semibold text-gray-900 mb-2">Summary</h5>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-gray-600">Total Income:</p>
                        <p class="text-lg font-bold text-green-600">${{ number_format($ttaxIncomes->sum('amount'), 2) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Total Expenses (Net):</p>
                        <p class="text-lg font-bold text-red-600">${{ number_format($ttaxExpenses->sum('net_ex_gst'), 2) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Total GST:</p>
                        <p class="text-lg font-bold text-blue-600">${{ number_format($ttaxExpenses->sum('gst_amount'), 2) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Net Profit:</p>
                        <p class="text-lg font-bold {{ ($ttaxIncomes->sum('amount') - $ttaxExpenses->sum('net_ex_gst')) >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            ${{ number_format($ttaxIncomes->sum('amount') - $ttaxExpenses->sum('net_ex_gst'), 2) }}
                        </p>
                    </div>
                </div>
            </div>
